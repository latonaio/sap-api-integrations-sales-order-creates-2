...
import (
	...
	sap_api_output_formatter "sap-api-integrations-sales-order-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callSalesOrderSrvAPIRequirementHeader("A_SalesOrder", header)
	if err != nil {
		c.log.Error(err)
		return
	}
